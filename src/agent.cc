...
// Print path in the format of (start_of_edge, end_of_edge, w=weight_of_edge), (s, e, w=), ...
...
void abm::Agent::move_agent(graph::weight_t time_limit) {
  if (path_.size() > 0) {
    // Keep record of cumulated edge cost (cumulative travel time) not to exceed the time_limit
    ...
    for (auto itr = path_.begin(); itr != path_.end() - 1; ++itr) {  
      ...
      if (agent_time < time_limit) {
        ...
        if (agent_time >= time_limit) {
          // If cumulative cost exceed the time_limit, make agent stops there (even though it does not finishing the current link)
  ...
  // When agent reachs destination, update its status to 2 "arrival" and delete the path
