...
TEST_CASE("Agent class is checked", "[agent]") {
  ...
  SECTION("Test agent initialization, parameter assignment and routing") {
    ...
    REQUIRE(test_agent -> get_current_node() == origin); // At first, the current node is the origin
    REQUIRE(test_agent -> get_status() == 0); // At first, the agent status is 0 "waiting to depart"
    test_agent -> compute_agent_path();
    // test_agent -> print_agent_path(); // (1, 2, w=1.5) (2, 4, w=5.5) (4, 3, w=0.2)
    
    // Check agent position after 2 units of cost
    test_agent -> move_agent(2);
    REQUIRE(test_agent -> get_current_node() == 2); // After 2 units of cost, agent can reach vertex 2
    REQUIRE(test_agent -> get_status() == 1); // At this time, agent status is 1 "enroute"
    
    // Check agent position after 10 units of cost
    test_agent -> move_agent(10);
    REQUIRE(test_agent -> get_current_node() == 3); // After 10 unit of cost, agent can reach vertex 3
    REQUIRE(test_agent -> get_status() == 2); // At this time, agent status is 2 "reached destination"
  }
}